...
with codecs.open(output_fpath, "w", "utf-8") as out:    
    for key, rows in grouped_df:
        try:
            pair_num += 1
            if pair_num % 10000 == 0: print pair_num,
            ...
            for i, row in rows.iterrows():
                ...
                if pair_num == 1: 
                    print >> out, ";".join([col for col in sum_row.index.values])
                num += 1
            sum_row.form = form
            sum_row.related = related        
            print >> out, ";".join([unicode(sum_row[col]) for col in sum_row.index.values])
        except:
            print format_exc()
